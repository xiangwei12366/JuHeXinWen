...
import com.xiangwei.souhu.dao.ChannelDao;
...
public class ChannelManage {
	 ...
	 * 获取其他的频道
	 * 
	 * @return 数据库存在的用户配置 ？数据库内的用户选择频道：默认用户选择频道
	 */
	public List<ChannelItem> getUserChannel() {
		Object cacheList = channelDao.listCache(SQLHelper.SELECTED + "=?",
				new String[] { "0" });
		if (channelDao != null && !((List) cacheList).isEmpty()) {
			userExist = true;
	 ...
	 * 获取其他的频道
	 * 
	 * @return 数据库存在的配置？数据库内的其他配置：默认其他频道
	 */
