...
import com.xiangwei.souhu.bean.CityEntity;
...
public class Constants {
	...
	public static ArrayList<NewsEntity> getNewsList() {
		...
		for (int i = 0; i < 10; i++) {
			...
			if (i <= 2) {
				news.setPublishTime(Long.valueOf(DataTools.getTime()));
			} else if (i > 2 && i <= 5) {
				news.setPublishTime(Long.valueOf(DataTools.getTime()) - 86400);
			} else {
				news.setPublishTime(Long.valueOf(DataTools.getTime()) - 86400 * 2);
			}
			newsList.add(news);
		}
		return newsList;
